...
<div style="display:flex;flex-wrap:wrap;justify-content:space-between;margin-left:50px; width:90%;">
	  ...
	  <b-field label="Шанс получить ранение во время войны">
		...
		<b-field label="Шанс на внезапное убийство основного героя">
            <b-slider v-model="assassinateChance" :custom-formatter="(val) => val + '%'" :tooltip="false" indicator style="width:300px"  :max="100"></b-slider>
        </b-field>
...
let statistic=[];
class Simulation{
	constructor(woundChance_war,woundChance_piece,humanitaryHelpChance,mercenaryHelpChance,startBjzi,startHumanitary,startGold,cureChance,assassinateChance) {
			...
			this.assassinateChance = assassinateChance;
			}
...
function assassinate(context){
	context.players.map(player=>{
		//console.log(player)
		let chance=getChance();
		let assassinated = chance>=context.assassinateChance?false:true;
		if (assassinated){player.state=-1;console.log('проверка на убийство',chance,context.assassinateChance,assassinated)};
		//console.log(player)
	});
	return context;
}
...
export default {
  ...
  mounted(){
	let simulation = new Simulation(this.woundChance_war,this.woundChance_piece,this.humanitaryHelpChance,this.mercenaryHelpChance,this.startBjzi,this.startHumanitary,this.startGold,this.cureChance,this.assassinateChance);
